...
# ╔═╡ 69e361da-f055-11f0-8871-674db69ba266
using WGLMakie, GeometryBasics, PlutoUI

# ╔═╡ c02f93cd-47d1-4d97-841d-384347dd28b7
@bind t PlutoUI.Slider(-2:0.001:2, default=0.000, show_value=true)

# ╔═╡ 48927e31-6d14-4339-a665-a37c73675ef3
begin
	res = 2500
	u = range(0f0, 2f0π; length=res)
	v = range(0f0, 2f0π; length=res)
	
	klein = [begin
	    x1 = sin(vv) * sin(uu/2f0)
	    x2 = sin(vv) * cos(uu/2f0)
	    x3 = (2f0 + cos(vv)) * sin(uu)
	    x4 = (2f0 + cos(vv)) * cos(uu)
	    (x1, x2, x3, x4)
	end for uu in u, vv in v]

	fig = Figure(size = (700, 500))
	ax = Axis3(fig[1, 1], aspect = :data, title = "Hyperplane section of the embedded Klein bottle at height $(round(t, digits=3))", titlesize = 20)

	function precise_eps(tt)
	    base_eps = 0.015f0
	    shrink = 0.012f0 * (exp(-50 * (abs(tt) - 1)^2) + exp(-50 * (abs(tt) - 3)^2))
	    return max(0.006f0, base_eps - shrink)
	end

	filtered_pts = lift(t) do tt
		curr_eps = precise_eps(tt)
		mask = abs.(getindex.(klein, 4) .- tt) .< curr_eps
		Point3f.(getindex.(klein[mask], 1), getindex.(klein[mask], 2), getindex.(klein[mask], 3))
	end

	meshscatter!(ax, filtered_pts, 
	    markersize = 0.05,
...
manifest_format = "2.0"
project_hash = "8e5c750bc11a94af317b2b814c4bd7df4e652956"
